...
private:
	...
	void AddQuad(Array<DefaultVertex>& vertices, std::vector<uint32_t>& indices, uint32_t i, uint32_t j, const glm::vec3& start, const glm::vec3& end, uint32_t axis, float direction, const glm::vec4& texCoords)
	{
        size_t offset = vertices.Count();
        ...
        vertices.Emplace(pos1 + glm::vec3(j * 2.0f - 1.0f, 0, -(i * 2.0f - 1.0f)), texCoords1, glm::vec3(normal));
        vertices.Emplace(pos2 + glm::vec3(j * 2.0f - 1.0f, 0, -(i * 2.0f - 1.0f)), texCoords2, glm::vec3(normal));
        vertices.Emplace(pos3 + glm::vec3(j * 2.0f - 1.0f, 0, -(i * 2.0f - 1.0f)), texCoords3, glm::vec3(normal));
        vertices.Emplace(pos4 + glm::vec3(j * 2.0f - 1.0f, 0, -(i * 2.0f - 1.0f)), texCoords4, glm::vec3(normal));
...
public:
	...
	Camera OnStart(KeyboardDevice& keyboard, MouseDevice& mouse) override
	{
		...
		AddSystem<DeferredRendererSystem>(glm::vec2(512));
		...
		ShaderHandle normalMappedShader = LoadShader("deferred/normalMapped_VS.glsl", "deferred/normalMapped_FS.glsl");
        ...
        ShaderHandle specularShader = LoadShader("deferred/specular_VS.glsl", "deferred/specular_FS.glsl");
        ...
        MeshHandle shadowFigureMesh = LoadMesh("Shadow Figure.obj");

        MaterialHandle blankMaterial = CreateMaterial(specularShader);
        blankMaterial->SetTexture("Texture", LoadTexture<glm::u8vec3>("blank.png"));


        MeshHandle doorMesh = LoadMesh("door.obj");
        MeshHandle  keyMesh = LoadMesh("key.obj");
		...
		for(uint32_t y = 1; y < levelBitmap->Height - 1; y++)
		{
			for(uint32_t x = 1; x < levelBitmap->Width - 1; x++)
			{
				...
				if(center != glm::u8vec3(0, 0, 0))
				{
                    ...
                    glm::quat doorRotation = glm::identity<glm::quat>();
                    ...
                    if(IsDoor(doorPixel))
                    {
                        ...
                        BaseBitmapHandle doorTexture = std::make_shared<Bitmap<glm::u8vec3>>(*rustMap, [=](uint8_t value)
                        {
                        ...
                        doorMaterial->SetTexture("Normal Map", doorNormalMap);
        ...
        {
            size_t randomIndex = size_t(std::randf() * shadowLocations.size());
        ...
        brickMaterial->SetTexture("bricks2_normal.jpg", normalsAtlas);
		...
		Camera player = CreateCamera(Transformation(glm::vec3(1.0f, PLAYER_HEIGHT, -1.0f)), Projection(70.0f, GraphicsMode().GetAspectRatio(), 0.01f, 10.0f));
...
GameSettings Game::StartSettings()
{
	...
	return GameSettings(GraphicsMode(1280, 720), "Nightmare Maze!", startScene, 5000);
}