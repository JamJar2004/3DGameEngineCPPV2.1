...
struct std::hash<Key>
{
    size_t operator()(const Key& key) const noexcept { return std::hash<uint32_t>()(static_cast<uint32_t>(key)); }
};
...
{
    size_t operator()(const MouseButton& button) const noexcept { return std::hash<uint32_t>()(uint32_t(button)); }
};
...
public:
    ...
    ButtonState GetButtonState(MouseButton button) const
    {
        ...
        if(m_heldButtons.contains(button))
        {
            return ButtonState::Held;
        }
        if(m_releasedButtons.contains(button))
        {
            return ButtonState::Released;
        }
        return ButtonState::None;
