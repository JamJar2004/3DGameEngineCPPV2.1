...
namespace GL
{
	TextureAttachment::TextureAttachment(uint32_t width, uint32_t height, size_t index, InternalImageFormat internalFormat, ImageFormat format, ElementType elementType, MinFilterMode minFilter, MagFilterMode magFilter, TextureWrappingMode wrappingMode) : 
	...
	void TextureAttachment::CopyTo(uint32_t xOffset, uint32_t yOffset, uint32_t width, uint32_t height, void* cpuBuffer)
	{
		...
		glReadPixels(xOffset, yOffset, width, height, GLenum(Attachment.Format), GLenum(Attachment.PixelElementType), cpuBuffer);
	}
	...
	FrameBuffer::FrameBuffer(uint32_t width, uint32_t height, const std::vector<AttachmentInfo>& colorAttachments, std::optional<AttachmentInfo> depthAttachment) : Width(width), Height(height)
	{
		...
		for(size_t i = 0; i < colorAttachments.size(); i++)
		{
			...
			auto& attachment = m_colorAttachments.emplace_back(width, height, i, attachmentInfo.InternalFormat, attachmentInfo.Format, attachmentInfo.PixelElementType, attachmentInfo.MinFilter, attachmentInfo.MagFilter, attachmentInfo.WrappingMode);
		...
		{
			m_depthAttachment = std::make_shared<Texture2D>(width, height, nullptr, depthAttachment.value().InternalFormat, depthAttachment.value().Format, depthAttachment.value().PixelElementType, depthAttachment.value().MinFilter, depthAttachment.value().MagFilter, depthAttachment.value().WrappingMode);
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, m_depthAttachment->m_ID, 0);
		}
		else
		{
			m_renderBuffer = std::make_shared<RenderBuffer>(width, height, InternalImageFormat::DepthComponent16);
	...
	{
		glBindFramebuffer(static_cast<GLenum>(mode), m_ID);
