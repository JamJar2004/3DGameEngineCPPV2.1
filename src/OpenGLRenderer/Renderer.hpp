...
namespace GL
{
	...
	template<size_t D>
	void ClearFloatBuffer(const glm::vec<D, glm::highp_float32>& value)
	{
		glClearBufferfv(GL_COLOR, 0, &value[0]);
	}

	template<size_t D>
	void ClearSIntBuffer(const glm::vec<D, glm::int32>& value)
	{
		glClearBufferiv(GL_COLOR, 0, &value[0]);
	}

	template<size_t D>
	void ClearUIntBuffer(const glm::vec<D, glm::uint>& value)
	{
