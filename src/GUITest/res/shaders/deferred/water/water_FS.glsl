...
void main()
{
	...
	vec2 distortion1 = texture(m_DistortionMap, v_texCoord + u_offset).rg * 2.0 - 1.0;
	vec2 distortion2 = texture(m_DistortionMap, v_texCoord - vec2(u_offset, -u_offset * 0.4)).rg * 2.0 - 1.0;
	...
	vec4 normalMapColor = texture(m_NormalMap, v_texCoord + u_offset + distortions);
