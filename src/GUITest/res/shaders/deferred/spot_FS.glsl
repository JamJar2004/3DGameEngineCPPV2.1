...
void main()
{
	...
	vec4  diffuseLightColor = (lightColor *  diffuseFactor * spotFactor) / attenuation;
	vec4 specularLightColor = (lightColor * specularFactor * spotFactor) / attenuation;
	
	vec4 shadowMapColor = texture(u_shadowMap, vec2(0.0));
	
	o_color = (texColor * diffuseLightColor + specularLightColor) * shadowFactor;//vec4(specularIntensity, specularIntensity, specularIntensity, 1.0);
}