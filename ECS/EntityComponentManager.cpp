...
namespace ECS
{
	...
	std::shared_ptr<BaseComponentArray> EntityComponentManager::GetComponentArray(const ComponentType& type)
	{
		...
		if(!result)
		{
			result = type.CreateArray(m_maxEntities);
	...
	void EntityComponentManager::AddComponent(EntityHandle handle, const BaseComponent& component)
	{
	...
	BaseComponent* EntityComponentManager::GetComponent(EntityHandle handle, const ComponentType& type) const
	{
