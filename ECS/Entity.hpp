...
namespace ECS
{
	...
	public:
		...
		const BaseComponent& operator*() const 
		{ 
			auto componentArray = m_manager->GetComponentArray(*m_iterator->first);
			return *componentArray->Get(m_iterator->second);
		}
	...
	public:
		...
		ComponentIterator begin() { return ComponentIterator(m_manager, m_manager->GetEntity(m_handle)->ComponentIndices.begin()); }
		ComponentIterator end()   { return ComponentIterator(m_manager, m_manager->GetEntity(m_handle)->ComponentIndices.end());   }
		
		ConstComponentIterator begin() const { return ConstComponentIterator(m_manager, m_manager->GetEntity(m_handle)->ComponentIndices.begin()); }
		ConstComponentIterator end()   const { return ConstComponentIterator(m_manager, m_manager->GetEntity(m_handle)->ComponentIndices.end());   }
	};
	...
	public:
		...
		size_t GetVersion() const { return m_handle.Version(); }
		      ...
		      ComponentCollection GetComponents()       { return ComponentCollection(m_manager, m_handle); }
		...
		template<std::convertible_to<const BaseComponent&> First, std::convertible_to<const BaseComponent&>... Rest>
	...
	template<typename... Args>
	inline void EntityEvent<Args...>::operator()(Entity& entity, Args... args)
	{
		entity.TriggerEvent<Args...>(*this, std::forward<Args>(args)...);
	}

	template<std::derived_from<BaseComponent> T>
	inline void Entity::AddComponent(const T& component) const requires std::copy_constructible<T>
	{
		m_manager->AddComponent(m_handle, component);
	}

	template<std::derived_from<BaseComponent> T, typename ...Args>
	inline T& Entity::AddComponent(Args&&... args) const requires std::constructible_from<T, Args...>
	{
	...
	{
		BaseComponent* result = m_manager->GetComponent(m_handle, Component<T>::Type);
		return *(T*)result;
	}
