...
struct AmbientLight : public Light
{
	...
	virtual Type* GetType() const override { return Type::Get<AmbientLight>(); }
};
...
struct DirectionalLight : public Light
{
	...
	virtual Type* GetType() const override { return Type::Get<DirectionalLight>(); }
...
struct PointLight : public Light
{
	PointLight(const glm::vec3& color, float intensity, const Attenuation& attenuation, ShadowInfoHandle shadowInfo = nullptr) :
		Light(color, intensity, shadowInfo), Attenuation(attenuation), Range(CalcRange(color, intensity, attenuation))
	{
	...
	virtual Type* GetType() const override { return Type::Get<PointLight>(); }

	virtual void UpdateShader(ShaderHandle shader) const override
	{
		Light::UpdateShader(shader);
		shader->Get<float>("u_constant") = Attenuation.Constant;
		shader->Get<float>("u_linear") = Attenuation.Linear;
		shader->Get<float>("u_exponent") = Attenuation.Exponent;
...
struct SpotLight : public PointLight
{
	...
	virtual Type* GetType() const override { return Type::Get<SpotLight>(); }
