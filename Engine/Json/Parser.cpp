...
namespace Json
{
	...
	Object* Parser::ParseObject()
	{
		...
		while(m_current->Type != TokenType::CloseObject && m_current->Type != TokenType::EndOfFile)
		{
			...
			std::string name = std::any_cast<std::string>(m_current->Value);
			++m_current;
	...
	Node* Parser::ParseNode()
	{
		...
		{
			result = new TypedValue<int64_t>(std::any_cast<int64_t>(m_current->Value));
			++m_current;
		}
		else if(m_current->Type == TokenType::FloatNumber)
		{
			result = new TypedValue<double>(std::any_cast<double>(m_current->Value));
			++m_current;
		}
		else if(m_current->Type == TokenType::String)
		{
			result = new TypedValue<std::string>(std::any_cast<std::string>(m_current->Value));
			++m_current;
		}
		else if(m_current->Type == TokenType::Boolean)
		{
			result = new TypedValue<bool>(std::any_cast<bool>(m_current->Value));
			++m_current;
		}
