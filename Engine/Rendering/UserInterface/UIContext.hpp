...
public:
	...
	void Pop()
	{
		Type* lastElementType = m_types[m_types.size() - 1];
		m_types.pop_back();
...
{
private:
	std::unordered_map<Type*, ArenaStack> m_styleStacks;
public:
	...
	{
		ArenaStack& stack = m_styleStacks[Type::Get<BaseStyle>()];
...
public:
	StackPanelContext(UIContext& context, ContainerContext& parent, Orientation orientation) : Context(context), Parent(parent), m_currentPosition(0.0f), Orientation(orientation) {}
